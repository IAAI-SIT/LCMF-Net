...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=512, depths=[9], 
                 dims=[512], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
        ...
        self.num_features = int(dims[0] * 2)
        self.dims = dims
        self.up = FinalPatchExpand_X4(dim_scale=4,dim=self.embed_dim*2)
        self.up_sample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.output = nn.Conv2d(in_channels=self.embed_dim*2,out_channels=self.embed_dim,kernel_size=1,bias=False)
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            ...
            x = layer(x)
        # print("lay_out:",x.shape)
        x = self.norm(x)  # B H W C
        return x, x_downsample

    def up_x4(self, x):
        # print("up_x4_in_shape:",x.shape)
        B,H,W,C = x.shape
        x = self.up(x)
...
if __name__ == "__main__":
    ...
    int = torch.randn(16,512,16,16).to('cuda:1')
    out = model(int)
    print(out.shape)
