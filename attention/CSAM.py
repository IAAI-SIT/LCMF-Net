...
def main():
    ...
    # 创建输入张量
    input_tensor = torch.randn(batch_size, num_slices, height, width)
    # 转移到 GPU（如果可用）
    device = torch.device('cuda:1' if torch.cuda.is_available() else 'cpu')
    # 实例化 CSAM 模型
    csam = CSAM(num_slices=batch_size*num_slices, num_channels=num_channels, semantic=True, positional=True, slice=True, uncertainty=True, rank=5)
    csam = csam.to(device)

    input_tensor = input_tensor.to(device)

    # 测试前向传播
    output = csam(input_tensor)
    print("输入的形状:", input_tensor.shape)
