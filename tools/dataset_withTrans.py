...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 应用数据增强
          # 应用数据增强
        if self.aug:
            # 将图像和标签转换为字典形式，传入变换函数
            augmented = self.transform(image=npimage.transpose(1, 2, 0), mask=nplabel.transpose(1, 2, 0))

            # 获取增强后的图像和标签，并转换回原来的形状
            npimage = augmented['image'].transpose(2, 0, 1)
            nplabel = augmented['mask'].transpose(2, 0, 1)
            # 打印增强后的图像和标签形状
