...
# train
train_set = {
    'root': 'F:/project/my-dataset/archive/Training_Data/',  # 四个模态数据所在地址
    'out': 'F:/project/my-dataset/archive/dataset/',  # 预处理输出地址
...
def process_h5(path, out_path):
    ...
    mask = images.sum(0) > 0    # 对像素值大于0的点生成一个布尔型的mask
    for k in range(4):
        x = images[k, ...]  # 将第k个图像中的所有维度，像素赋值给x相当于整个图像copy
        y = x[mask]     # y等于图像中像素值不为0的区域，y即目标区域

        # 对背景外的区域进行归一化
        x[mask] -= y.mean()
        x[mask] /= y.std()      # y.std()为求出目标区域内的标准差

        images[k, ...] = x      # 将处理好的图像重新赋值给原图像
