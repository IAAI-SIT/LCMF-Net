...
def iou_score(output, target):
    ...
    for i in range(num_classes):
        output_channel = torch.sigmoid(output[:, i, :, :]).data.cpu().numpy()
        target_channel = target[:, i, :, :].data.cpu().numpy()

        output_channel = (output_channel > 0.5).astype(np.float32)
        intersection = np.sum(output_channel * target_channel)
        union = np.sum(output_channel) + np.sum(target_channel) - intersection
